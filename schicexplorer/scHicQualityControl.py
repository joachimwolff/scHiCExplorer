...
def compute_contains_all_chromosomes(pMatrixName, pMatricesList, pChromosomes, pQueue):
    ...
    for matrix in pMatricesList:
        ...
        try:
            ...
            # log.debug('should: {} counted {}'.format(len(args.chromosomes) , count_chromosomes))
            # log.debug('Message: {}'.format(error))
            keep_matrices_chromosome_names.append(0)
    # log.debug('keep_matrices_chromosome_names {}'.format(keep_matrices_chromosome_names))
...
def main(args=None):
    ...
    if args.chromosomes:
        ...
        # keep_matrices_chromosome_names = np.array(keep_matrices_chromosome_names)
